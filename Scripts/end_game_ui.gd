...
var saved_highest_score: HighestScore
...
func set_highest_score() -> void:
	highest_score_label.text = "Highest Score: " + str(load_highest_score().player_high_score[PlayerScore.game_mode])
...
func save_highest_score() -> void:
	if not saved_highest_score.player_high_score.has(PlayerScore.game_mode):
		saved_highest_score.player_high_score[PlayerScore.game_mode] = PlayerScore.player_score
	elif saved_highest_score.player_high_score[PlayerScore.game_mode] < PlayerScore.player_score:
		saved_highest_score.player_high_score[PlayerScore.game_mode] = PlayerScore.player_score
	ResourceSaver.save(saved_highest_score, "user://highest_score.tres")

func load_highest_score():
	if not ResourceLoader.exists("user://highest_score.tres"):
		return HighestScore.new()
	var highest_score_stored: HighestScore = ResourceLoader.load("user://highest_score.tres", "HighestScore")
	return highest_score_stored

func _on_replay_button_pressed() -> void:
	match PlayerScore.game_mode:
		GameMode.GameModeEnum.NORMAL:
			get_tree().change_scene_to_file("res://Scenes/Game/GameNormal.tscn")
		GameMode.GameModeEnum.HARDCORE:
			get_tree().change_scene_to_file("res://Scenes/Game/GameHardcore.tscn")
		GameMode.GameModeEnum.INFINITY:
			get_tree().change_scene_to_file("res://Scenes/Game/GameInfinite.tscn")
